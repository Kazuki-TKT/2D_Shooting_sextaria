...
public class STG_DeadArea : MonoBehaviour
{
    ...
    private void HitCheck(Transform colTrans)
    {
        ...
        else if (goName.Contains(UbhEnemy.NAME_PLAYER) == false)//Playerじゃなかったら
        {
            if (goName.Contains("Item")|| goName.Contains("Tutorial") || goName.Contains("Boss"))
            {
                return;
            }
            //Debug.Log($"<color=#fff5ee><b>DeadArea</b></color>" + goName);
